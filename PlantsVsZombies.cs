...
public class PlantsVsZombies : InjectEffectPack
{
    ...
    private void InitGame()
    {
        ...
        byte[] top_class_pattern =
        [
            0x8B, 0x0D, 0xAF, 0xAF, 0xAF, 0xAF, 0x8B, 0x89, 0xAF, 0xAF, 0xAF, 0xAF, 0x83, 0xF9, 0x14
        ];
        ...
        byte[] collect_pattern =
        [
            0x75, 0xAF, 0x8B, 0xAF, 0xE8, 0xAF, 0xAF, 0xAF, 0xAF, 0xEB, 0xAF, 0x8B, 0xAF, 0xE8, 0xAF, 0xAF, 0xAF, 0xAF, 0x83
        ];
        ...
        switch (codeParams[0])
        {
            ...
            case "nocooldown":
            {
                ...
                if (cards_ptr_ch.GetInt() != 0)
                {
                    ...
                    act.WhenCompleted.Then(_ => { Connector.SendMessage("No cooldown ended."); });
                }
                    ...
                    var act = RepeatAction(request,
                        ...
                        () =>
                        {
                            for (int i = 0; i < ncards; i++)
                            {
                                ...
                                if (is_up)
                                {
                                    new_cooldown = max_cooldown + random_big_percentage(max_cooldown);
                                }
                                else
                                {
                                    new_cooldown = max_cooldown - random_big_percentage(max_cooldown);
            ...
            case "plantanywhere":
            {
                if (collision_ch.GetByte() == JZ)
                {
                    DelayEffect(request, StandardErrors.AlreadyInState);
                    return;
                }
            ...
            case "invinciblezombies":
            {
                ...
                var tim = StartTimed(request,
                    () => (invincible_zombies_ch.GetByte() != NOP),
                    is_not_paused,
                    TimeSpan.FromMilliseconds(500),
                    () =>
                    {
            ...
            {
                var tim = StartTimed(request,
                    () => (GetUShort(slow_bullets_ch) != NOP_NOP),
                    is_not_paused,
                    TimeSpan.FromMilliseconds(500),
                    () =>
                    {
            ...
            {
                var tim = StartTimed(request,
                    () => (high_gravity_bullets_ch.GetByte() != NOP),
                    is_not_paused,
                    TimeSpan.FromMilliseconds(500),
                    () =>
                    {
            ...
            {
                var tim = StartTimed(request,
                    () => (backwards_bullets_ch.GetByte() != NOP),
                    is_not_paused,
                    TimeSpan.FromMilliseconds(500),
                    () =>
                    {
            ...
            {
                var tim = StartTimed(request,
                    () => (invincible_plants_ch1.GetByte() != NOP),
                    is_not_paused,
                    TimeSpan.FromMilliseconds(500),
                    () =>
                    {
            ...
            case "zombiessize":
            {
                ...
                if (active_zombies_ptr.GetInt() != 0 && nactive_zombies > 0)
                {
                    ...
                    if (!is_one_zombie_in_visible_range(active_zombies_ch, nactive_zombies))
                    {
                        DelayEffect(request, StandardErrors.NoValidTargets);
                        return;
                    }
            ...
            case "zombiesspeed":
            {
                ...
                byte[] t1 =
                [
                    0xC7, 0x43, 0x08, 0x00, 0x00, 0x00, 0x00, 0xD8, 0x4B, 0x08, 0x5B, 0xD9, 0x5C, 0x24, 0x04, 0xD9, 0x44, 0x24, 0x04, 0x83, 0xC4, 0x14, 0xC3
                ];
                switch (codeParams[1])
                {
                    case "faster": // 50.0
                ...
                tim.WhenCompleted.Then(_ =>
                {
                    byte[] t2 =
                    [
                        0xD8, 0x4B, 0x08, 0x5B, 0xD9, 0x5C, 0x24, 0x04, 0xD9, 0x44, 0x24, 0x04, 0x83, 0xC4, 0x14, 0xC3
                    ];
            ...
            case "zombiesmiddle":
            {
                ...
                if (active_zombies_ptr.GetInt() != 0 && nactive_zombies > 0)
                {
                    ...
                    act.WhenCompleted.Then(_ => { Connector.SendMessage("Zombies returned to their lanes."); });
                }
            ...
            case "teleportzombies":
            {
                ...
                if (active_zombies_ptr.GetInt() != 0 && nactive_zombies > 0)
                {
                    ...
                    act.WhenCompleted.Then(_ => { Connector.SendMessage("Teleporting zombies ended."); });
                }
